...
The restricted bash tool permits both `pedelec-deno --thread-id <pedelec_thread_id> run <workspace-relative-script-path>` for workspace-file execution and `pedelec-deno --thread-id <pedelec_thread_id> run -` for source supplied through the bash tool's optional `stdin` field. Either execution target may be followed by `-- <script-args...>`. `pedelec-deno` is the canonical JavaScript/TypeScript runtime for this session. Do not substitute system-installed Node.js, Bun, raw Deno, npx, or another JavaScript runtime, and do not silently fall back if `pedelec-deno` is unavailable. App Tool calls remain `pedelec-cli`; general shell access remains unavailable.\n\n\
