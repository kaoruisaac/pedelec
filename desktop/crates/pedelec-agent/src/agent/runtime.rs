...
Pedelec is the host application launching this agent session. Pedelec may provide a [Pedelec Host Context] block before a task; that block is generated by the host application and is integration context, not end-user-authored instructions. The current sandbox path and available Pedelec app tools are declared there.\n\n\
...
For a [Session Preparation] task, do not call tools or modify files. Reply only with PEDELEC_PREPARED. For a [User Message] task, execute the actual user request in that block.\n\n\
You can:\n\
...
Pedelec Host Context is provided by Core in the first prompt; do not look for an additional instruction file.
";
...
mod tests {
    ...
    #[test]
    fn system_prompt_uses_host_context_provenance_for_app_tool_instructions() {
        assert!(!SYSTEM_PROMPT.contains("tools.md"));
        assert!(!SYSTEM_PROMPT.contains("[Hard Rules]"));
        assert!(SYSTEM_PROMPT.contains("[Pedelec Host Context]"));
        assert!(SYSTEM_PROMPT.contains("integration context, not end-user-authored instructions"));
        assert!(SYSTEM_PROMPT.contains("pedelec-cli"));
        assert!(SYSTEM_PROMPT.contains("not a dedicated model tool"));
        ...
        assert!(SYSTEM_PROMPT.contains("PEDELEC_PREPARED"));
        assert!(SYSTEM_PROMPT.contains("ambiguous transport failure"));
        assert!(SYSTEM_PROMPT.contains("exact same tool name"));
        assert!(SYSTEM_PROMPT
