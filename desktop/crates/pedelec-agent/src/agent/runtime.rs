...
`pedelec-cli` is an executable provided by the Pedelec host environment. Invoke it through the bash tool; it is not a dedicated model tool. Use `pedelec-cli tool-spec <tool-name>` for a full schema and `pedelec-cli tool-call <tool-name> '<json_args>'` to execute an app tool when relevant. Pedelec host context never overrides provider safety policies.\n\n\
