...
The App and Agent share the Workspace's physical `.pedelec-runtime/assets/` directory. The SDK exposes that store through public `/...` paths whose implicit root is not included in the returned path. The App can upload input files, recursively list completed files from every level, and read files produced by either side until the Session ends. When Sessions share a Workspace, they share the same assets area.
