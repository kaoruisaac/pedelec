...
The App and Agent share the session sandbox's `assets/` directory. The App can upload input files, list completed top-level files, and read files produced by either side until the session ends.

### Upload a file

Use `uploadAsset()` with one browser `File`. Files are limited to 100 MiB; the returned `assets/...` value is a sandbox-relative path and is only valid for that session's lifetime. Upload can run alongside prepare and agent execution, although a session allows only one concurrent upload.
...
`listAssets()` returns regular files from the first level of `assets/`, sorted by filesystem modification time newest first. It is non-recursive, has no pagination, and can be called while the Agent runs. Use a known path with `readAsset()` when the Agent writes a nested file.
...
const modelFile = await session.readAsset("assets/model.glb", "file");
```

`"text"` requires valid UTF-8. `"json"` also parses the text as JSON; its generic type is only a TypeScript annotation, so validate untrusted data before using it. `"file"` preserves binary bytes in a browser `File`. Reads support known nested `assets/...` paths and files up to 100 MiB.
