...
The App and Agent share the session sandbox's `assets/` directory. The App can upload input files, recursively list completed files from every level, and read files produced by either side until the session ends.
