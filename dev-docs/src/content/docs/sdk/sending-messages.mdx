...
Use `uploadAsset()` with one browser `File`. Files are limited to 100 MiB; returned public paths use `/...`, with the sandbox `assets/` directory as their implicit root. Upload can run alongside prepare and agent execution, although a session allows only one concurrent upload.
