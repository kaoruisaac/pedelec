...
The App and Agent share the session's physical `.pedelec-sandbox/assets/` directory. The SDK exposes that store through public `/...` paths whose implicit root is not included in the returned path. The App can upload input files, recursively list completed files from every level, and read files produced by either side until the session ends.

### Upload a file

Use `uploadAsset()` with one browser `File`. Files are limited to 100 MiB; returned public paths use `/...`, with `.pedelec-sandbox/assets/` as their physical root and `assets/` as their implicit SDK root. Upload can run alongside prepare and agent execution, although a session allows only one concurrent upload.
