...
Use `uploadAsset()` with one browser `File` before the session ends. Files are limited to 100 MiB; the returned `assets/...` value is a sandbox-relative path and is only valid for that session's lifetime. Upload can run alongside prepare and agent execution, although a session allows only one concurrent upload.
...
`listAssets()` is non-recursive and sorted by filesystem modification time, newest first. It lists shared `assets/` files created either by the App or Agent. `readAsset(path, "text" | "json" | "file")` reads known nested paths up to 100 MiB; binary bytes use the loopback ticket data plane. Listing and reading are allowed while the agent runs.
