...
`entry` is resolved by Vite at build/dev time; it is not sent to Desktop. The plugin bundles local/package dependencies and generates public declarations. The Agent imports `sprite-tools` from a script run with `pedelec-deno`; Host Context points exact API questions to `index.d.ts`. Module code shares the restricted runtime and cannot fetch packages or network resources at execution time. Deno Modules are distinct from browser/App Tools in `skills.tools`, which use `pedelec-cli` RPC commands. Session end/resume keeps the same immutable snapshot while Core still knows the thread; create a new session to use a rebuilt artifact.
