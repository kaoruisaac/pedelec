...
The picker returns a path or `null` on cancellation. It does not decide whether the folder is a valid Pedelec sandbox or initialize its contents; Core applies the existing sandbox rules during `createSession()`.
