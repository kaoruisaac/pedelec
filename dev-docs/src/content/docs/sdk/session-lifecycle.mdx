...
4. reconciles the returned authoritative lifecycle snapshot, including status, active/last operation identity, and a pending tool request when present;
...
## Provider and effort metadata after resume
...
If the UI needs these fields after reload, save them with the session ID or load them from application-owned persistence. Do not assume `resumeSession()` rehydrates all metadata, but do rely on `getStatus()` for the Core lifecycle state after resume resolves.
