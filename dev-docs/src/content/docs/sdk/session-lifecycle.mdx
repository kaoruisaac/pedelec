...
`end()` does not immediately delete the session sandbox. It is temporary Desktop-managed storage, not durable application storage. The Desktop App removes sandboxes on normal app exit and attempts to clean up leftovers at the next launch after an abnormal termination. Closing the main window only hides the app and does not run this cleanup. The ended handle cannot access assets through `uploadAsset()`, `listAssets()`, or `readAsset()`, even before cleanup runs. Locked files can leave cleanup for a later launch.
