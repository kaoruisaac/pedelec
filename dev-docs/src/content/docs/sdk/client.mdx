...
It does not create/resume sessions or open approval. It skips Desktop probing for unapproved origins; for approved ones it uses `getSettings()`, which can trigger the established auto-launch fallback. `launchAttempted` means that probe was sent, not that Desktop definitely launched; settings-shape failures also make the probe unavailable.
