...
`openWorkspace()` without a path is the exception: while the native picker is open, it intentionally has no SDK bridge wall-clock timeout. An explicit-path `openWorkspace(path)` uses the configured bridge timeout. Extension/native disconnects and explicit bridge errors still reject picker requests immediately. `workspace.run()` also delegates timeout authority to Core so long-running scripts are not cut off by the generic bridge timer.
...
Continue with [Providers and effort profiles](./providers-and-models/).
