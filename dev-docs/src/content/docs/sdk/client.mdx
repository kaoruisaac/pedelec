...
`directoryPicker()` is the exception: while the native picker is open, it intentionally has no SDK bridge wall-clock timeout. Extension/native disconnects and explicit bridge errors still reject it immediately.
