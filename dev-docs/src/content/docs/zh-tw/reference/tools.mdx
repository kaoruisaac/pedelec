...
`guidance` 必須是 string，`tools` 必須是 array。`denoModules` 是 optional；每個 module 都需要 `name`、`description`、`entry`、`usage`。它應包含 Vite-prepared definitions，`createSession()` 會自動完成 transport。
