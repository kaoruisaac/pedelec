...
Core snapshot 在已知時也會包含這個值，因此 `resumeSession()` 可以 hydrate usage。Provider 或 payload 不支援有效 total 時則維持 `undefined`；目前 Cursor ACP 即屬於此情況。這個值不是 context-window occupancy、金額 cost，也不保證等同 invoice；目前 API 也不提供 input/output/cache/reasoning breakdown。
