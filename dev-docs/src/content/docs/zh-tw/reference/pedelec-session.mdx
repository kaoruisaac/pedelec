...
回傳 App 與 Agent 共用的第一層 `assets/`；不遞迴，依 filesystem modification time 由新到舊排序。`readAsset(path, "text")`、`readAsset(path, "json")` 與 `readAsset(path, "file")` 可讀取已知 nested asset path；text 使用 strict UTF-8，JSON generic 僅提供 TypeScript typing。
