...
以 flat array 回傳 App 與 Agent 共用 `assets/` 目錄所有層級中已完成的一般檔案。依 filesystem modification time 由新到舊排序；時間相同時依 `name` 排序。資料夾項目不會回傳，file 與 directory symlink 也不會回傳或被跟隨。所有層級 basename 以 `.pedelec-` 開頭的 entry 都會排除；其他 dotfile 與 dot-directory 中的檔案會列出。
...
讀取 shared session `assets/` 中的已知檔案，也可使用 `/results/report.json` 之類的 nested public path。Path 必須以 `/` 開頭、使用 `/`，且不能包含空 segment、`.` 或 `..`。
...
呼叫 `end()` 會結束 session，但不會立即刪除其 sandbox。未指定 `sandbox.path` 時，session 使用 Desktop 管理的暫存資料：Desktop App 正常退出時清除 managed sandbox，異常終止遺留的內容會在下次啟動時嘗試清除。指定 absolute `sandbox.path` 時，workspace 由 application 管理，Pedelec 在 session 結束、App 退出或 stale cleanup 時都不會刪除它，多個 active session 可以共用。關閉主視窗只會隱藏 App，不會觸發 managed cleanup。即使 sandbox 仍存在，ended session 也不能使用 `uploadAsset()`、`listAssets()` 或 `readAsset()`；shared workspace 的 filesystem conflict 由 application 負責，explicit path 不得與 managed sandbox root overlap。
