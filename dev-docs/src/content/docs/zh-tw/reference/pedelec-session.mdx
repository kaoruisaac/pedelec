...
Session 尚未結束時可 upload 一個 browser `File`。upload 可與 prepare、send 和 provider execution 同時進行，但每個 session 只允許一個 upload。單檔上限 100 MiB；成功時回傳 `assets/...` sandbox-relative path。
...
讀取 shared session `assets/` 中的已知檔案。和 `listAssets()` 不同，這個 method 可接受 `assets/results/report.json` 之類的 nested path。Path 必須以 `assets/` 開頭、使用 `/`，且不能包含空 segment、`.` 或 `..`。
