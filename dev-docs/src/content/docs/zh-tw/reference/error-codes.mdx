...
| `MODEL_REQUIRED` | Provider（特別是 Ollama）要求選中的 Desktop effort profile 有 model，且沒有提供 explicit `model` override | 到 Desktop Settings 設定 profile model，或傳入 `createSession({ model })` | 設定或修改 request 後 |
