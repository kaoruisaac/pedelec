...
| `TOOL_TIMEOUT` | Core 等待 tool result 超過 timeout | Core/runtime error | 不要自動 retry；先 reconcile side effect |
...
| `ASSET_PATH_INVALID` | Path 不在 `assets/` 或含不安全 segment | 使用有效的 `assets/...` relative path |
