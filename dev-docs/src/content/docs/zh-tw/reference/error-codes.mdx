...
- provider discovery；
- session ID 尚未回傳前就失敗的 creation；
- 已知尚未執行的 read-only tool。

### Retry 前必須 reconcile

- transport failure 後的 `sendText()`，因 provider execution 可能已開始；
- mutation tool result delivery failure；
- Core state 不確定的 `end()`；
- local work 可能仍完成的 timeout。

### 不可 unchanged retry

- `INVALID_INPUT`；
- 沒加 model 的 `MODEL_REQUIRED`；
- 沒設定的 `DEFAULT_PROVIDER_NOT_SET`；
- 同一 handle 的 `SESSION_ENDED`；
- 沒有改變 state/user intent 的 permission/domain error。
...
function messageForError(error: PedelecError) {
  switch (error.code) {
    ...
    case "NATIVE_HOST_UNAVAILABLE":
      return "請啟動或修復 Pedelec Desktop App。";
    default:
...
不要丟棄 unknown `code`、`message` 或安全的 diagnostic details；可能來自 provider 或較新的 runtime。
...
- 有的話 `sessionId`；
- provider/model；
- event/tool error 的 `turnId`、`toolRequestId`；
- component versions；
- error source 是 `core` 或 `sdk`。

不要預設記錄完整 prompts、selected page data、tool args 或 results。
