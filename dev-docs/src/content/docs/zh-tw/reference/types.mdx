...
`entry` 是 Vite build/dev authoring string。`name` 會成為 Agent 的 import specifier，`usage` 是必要的常見 import guidance。請使用 `pedelecVitePlugin()` 準備 runtime 與 declaration artifact。
