...
這是 `sandboxFolderPicker()` 回傳的 read-only snapshot。`hasSandboxConfig` 只表示 `.pedelec-sandbox.json` 是 regular file，不會驗證 marker 內容。
...
Session shared `assets/` 目錄中的 sandbox-relative path。Runtime validation 也會拒絕 backslash、空 path segment、`.` 與 `..`。
