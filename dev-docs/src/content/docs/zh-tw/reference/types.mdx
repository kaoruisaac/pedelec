...
`name` 與 `entry` 是 intrinsic fields。`entry` 是 Vite build/dev authoring string，不是 Desktop filesystem instruction。`description` 與 `usage` 是 optional Agent-facing metadata；`preferStdinExecution` 是 optional Agent execution preference。設為 `true` 時，Host Context 會提供帶有 `<typescript-source>` placeholder 的 module-specific stdin `runCommandTemplate`。File-backed execution 仍然有效，且此 preference 不會改變 bundling 或 artifact contents。請使用 `pedelecVitePlugin()` 準備 runtime 與 declaration artifact。
