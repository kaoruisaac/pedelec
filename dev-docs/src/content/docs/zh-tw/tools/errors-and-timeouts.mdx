...
當 `pedelec-cli` 偵測到 `tool-call` 的 IPC/transport failure 時，回傳的 structured error 會附上簡短指示，要求對同一個 listed call command 使用語意完全相同的 arguments exact retry。成功呼叫、一般 structured App Tool error 與正式的 `TOOL_TIMEOUT` response 不會附加這個 transport-retry 指示。
