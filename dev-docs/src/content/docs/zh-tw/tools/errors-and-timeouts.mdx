...
Provider 的 shell 或 terminal 可能在 Core 的 `timeoutMs` deadline 前停止等待 `pedelec-cli tool-call`。這是 ambiguous transport failure，不代表 App Tool 已失敗。若 Agent 沒有收到完整 structured Pedelec response，可以用完全相同的 tool name 與語意完全相同的 arguments exact retry；Core 會 join 仍在執行的 invocation，或 replay 最近完成但尚未確認 delivery 的 result。不要改 arguments，也不要無限 retry。
