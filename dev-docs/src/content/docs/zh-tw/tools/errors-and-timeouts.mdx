...
Provider 的 shell 或 terminal 可能在 Core 的 `timeoutMs` deadline 前停止等待 `pedelec-cli --thread-id <pedelec_thread_id> tool-call`。這是 ambiguous transport failure，不代表 App Tool 已失敗。若 `pedelec-cli` 本身在收到 structured Core response 前偵測到 IPC/transport failure，會自動使用相同 request identity 與 payload 立即 retry 一次。Core 可 join 仍在執行的 invocation，或 replay 最近完成但尚未確認 delivery 的 result，因此 recovery 不會刻意以不同語意 dispatch 第二個 invocation。若 provider 在 CLI 執行內部 retry 前強制終止 `pedelec-cli` process，則無法透過此機制恢復。

Structured Core response 對該次 CLI invocation 都是 final，包括 `TOOL_TIMEOUT` 及其他 App Tool/Core error。Agent 應使用回傳的 structured result 或 error；Host Context 不再要求 Agent 對未完整收到的 `pedelec-cli` call exact retry。

Core 到達 App Tool 的 `timeoutMs` 時，會回傳正式的 `TOOL_TIMEOUT` structured error。收到 `TOOL_TIMEOUT` 代表原 invocation 已結束；CLI 不會 retry 這個 structured response，也不能把它當成繼續同一 invocation 的指示。只有 IPC delivery 未確認的 result 可能在短時間內 replay；成功 delivery 的 result 不會作為一般 deduplication cache 保留。
...
| `pedelec-cli` 在收到 structured response 前偵測到 ambiguous Core IPC/transport failure | CLI 使用相同 request 立即內部 retry 一次；Core 可 join 或 replay 該 invocation |
| Provider 在 CLI 內部 retry 前強制終止 `pedelec-cli` | CLI 無法恢復；將未完成的 command result 視為 ambiguous |
| 已收到 structured `TOOL_TIMEOUT` 或其他 Core/App Tool error | CLI 不會 retry；適用時先 reconcile 可能的 side effect |
