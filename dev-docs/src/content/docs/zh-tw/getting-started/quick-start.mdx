...
只提供 `provider` 時，SDK 使用 `default` effort；Core 從 Desktop Settings 解析 profile。可以在有無 provider 時傳入 `effortLevel: "low"` 或 `"high"`，也可以傳入 provider-native `model` 做單一 session override：
...
省略 model 時行為不變；explicit model 只取代 Desktop profile 的 model，native effort 仍保留。Ollama 選中的 profile 沒有 model 時，explicit override 可滿足需求；否則會回傳 `MODEL_REQUIRED`，且不會從 Low/High fallback 到 Default。
