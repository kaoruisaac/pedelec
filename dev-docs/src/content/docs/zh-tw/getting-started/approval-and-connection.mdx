...
`getApprovalStatus()` 只讀取狀態並 ping Core，不會主動提出核准要求。`getSettings()` 與 `listProviders()` 是敏感 Desktop API，需 origin approval 且可能開啟 popup；前者只回傳 defaults，後者只回傳 `name`/`code`/`available`/`error`。請用 `appConnected`，不要再以 `listProviders()` 作為 connectivity probe。
