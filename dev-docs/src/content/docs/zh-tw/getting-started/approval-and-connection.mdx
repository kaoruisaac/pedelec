...
Pedelec 不允許任意網站自動建立本機 agent session 或開啟原生 directory picker。Chrome Extension 會辨識 page origin，並在該 origin 第一次呼叫 `createSession()`、`resumeSession()` 或 `openWorkspace()` 前要求使用者核准。
...
`getApprovalStatus()` 只讀取狀態並 ping Core，不會主動提出核准要求。`getSettings()`、`listProviders()` 與 `openWorkspace()` 是敏感 Desktop API，需 origin approval 且可能開啟 popup；它們只回傳 defaults、provider summary，或已初始化的 workspace capability。請用 `appConnected`，不要再以 `listProviders()` 作為 connectivity probe。

尚未核准時，第一個建立/resume session 或開啟 workspace 的操作才會觸發：
