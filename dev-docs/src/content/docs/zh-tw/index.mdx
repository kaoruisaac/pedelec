...
上圖是 control/event path。Asset file body 使用另一條內部路徑：`Web App fetch(PUT) → Desktop-managed 127.0.0.1 loopback upload server → session sandbox input/`。只使用 `session.uploadAsset()`，不要依賴內部 URL、port、token 或 ticket lifecycle。
...
Error event 會在 event 頂層（不在 `error` object 內）附帶責任領域。`source` 為 `"provider"` 或 `"core"`。provider error 一定會包含 `provider`；目前合法的 provider code 為 `"codex"`、`"antigravity"`、`"opencode"`、`"cursor"`、`"claude"` 與 `"ollama"`。core error 不會包含 `provider`。
