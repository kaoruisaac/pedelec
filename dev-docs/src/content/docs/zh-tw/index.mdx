...
上圖是 control/event path。Asset file body 使用另一條內部路徑：`Web App fetch(PUT/GET) → Desktop-managed 127.0.0.1 loopback asset transfer server → session .pedelec-sandbox/assets/`。只使用 asset API，不要依賴內部 URL、port、token 或 ticket lifecycle。
