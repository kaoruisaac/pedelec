...
Ollama session 使用 Desktop 設定的 endpoint 與內附 `pedelec-agent`。選中的 effort profile 必須有 model，除非 `createSession({ model })` 提供 explicit override；兩者皆沒有時空 profile 會回傳 `MODEL_REQUIRED`，不會 fallback 到 `default`。
...
這會只讀取 Desktop App 的 `defaultProvider`。如果沒有設定 default provider，SDK 會拋出 `DEFAULT_PROVIDER_NOT_SET`；具體 model 與 effort argv 由 Core 解析 Desktop 設定。若傳入 `model`，Core 只取代該 profile 的 model，仍保留 native effort 設定。
...
只傳 `provider` 時，SDK 使用 `effortLevel: "default"`。非 Ollama provider 可使用空 profile；Ollama 的選中 profile 沒有 model 時會回傳 `MODEL_REQUIRED`，除非同時傳入 explicit `model`。

### 指定 model override

`model` 是 optional provider-native identifier，可以和 `provider`、`effortLevel` 一起傳入：
...
省略 `model` 時行為不變；提供時只取代選中 Desktop profile 的 model，其他 native effort 設定保持不變。Core 會在建立時將結果 snapshot 到 thread 的 `effort_args`，之後修改 Desktop Settings 或 resume 都不會改變這份 session 設定。SDK 不維護 model catalog，identifier 是否有效仍由 provider/account 決定。
