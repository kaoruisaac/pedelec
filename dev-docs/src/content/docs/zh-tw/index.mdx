...
Pedelec 是一套 browser SDK 與本機橋接架構，讓 Web App 可以和 Codex、Gemini、OpenCode、Cursor、Claude Code、Grok，或使用 Ollama 的 agent 協作。
...
4. 目標 provider 在使用者本機可用。CLI 型 provider 使用 `codex`、`gemini`、`opencode`、`cursor`、`claude` 或官方 `grok`；Ollama provider 使用 Pedelec 隨附的 `pedelec-agent`。
