...
Ollama session 使用 Desktop 設定的 endpoint 與內附 `pedelec-agent`。Profile mode 的選中 effort profile 必須有 model，否則空 profile 會回傳 `MODEL_REQUIRED`，不會 fallback 到 `default`；explicit `{ model }` mode 直接提供 model，Ollama 目前不接受 explicit `effort`。
...
這會只讀取 Desktop App 的 `defaultProvider`。如果沒有設定 default provider，SDK 會拋出 `DEFAULT_PROVIDER_NOT_SET`；profile mode 的具體 model 與 effort argv 由 Core 解析 Desktop 設定。Explicit model mode 不讀取或合併 Desktop profile。
...
只傳 `provider` 時，SDK 使用 profile mode 的 `effortLevel: "default"`。非 Ollama provider 可使用空 profile；Ollama 的選中 profile 沒有 model 時會回傳 `MODEL_REQUIRED`。若要直接指定 model，請使用 explicit mode。

### 指定 explicit model 與 effort

`model` 是 explicit provider-native identifier，可以搭配 optional `effort`，但不可和 `effortLevel` 一起傳入：
...
只有 `model` 時使用 provider default effort；explicit 設定不讀取或合併 profile，並由 Core snapshot 到 thread 的 `effort_args`。之後修改 Desktop Settings 或 resume 都不會改變這份 session 設定。SDK 不維護 model catalog，model/effort 是否有效仍由 provider/account 決定。
...
`getSettings()` 與 `listProviders()` 需要 origin approval 且可能開啟 popup。Settings 絕不公開 provider credentials，而 provider entry 僅含上述欄位；SDK 會忽略 Extension response 中新增的未知欄位，以保持 forward compatibility。請以 `getApprovalStatus().appConnected` 做非敏感 Desktop connectivity probe。明確傳入 `model` 時，SDK 會要求 Core 回傳 `explicitModelConfigApplied: true`；舊版元件會被辨識為不相容，不會靜默套用其他 mode。
...
結束 session 不會立即刪除其 workspace。使用 `pedelec.createSession()` 建立的 session 使用 Desktop 管理的暫存資料：Desktop App 正常退出時會清除 managed workspace，異常結束後的殘留會在下次啟動時清理。Application 管理的 workspace 透過 `pedelec.openWorkspace(path)` 開啟，再使用 `workspace.createSession()`；Pedelec 在 session 結束、App 退出或 stale cleanup 時都不會刪除它。關閉主視窗只會隱藏 App，不會觸發 managed cleanup。Ended session 不能透過 asset API 存取保留的 workspace，explicit shared workspace 的 filesystem conflict 也不由 Pedelec 協調。
...
- 從 [Pedelec 的 Deno](./deno/overview/)選擇 Agent script、Deno Module 或 `workspace.run()`。
