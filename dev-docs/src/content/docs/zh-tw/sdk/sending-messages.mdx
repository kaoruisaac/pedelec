...
App 與 Agent 共用 session sandbox 的 `assets/` 目錄。Session 結束前，App 可以 upload input file、列出第一層已完成檔案，並讀取任一方產生的檔案。

### Upload 檔案

使用 `uploadAsset()` 傳送一個 browser `File`。單檔上限為 100 MiB；回傳的 `assets/...` 是 sandbox-relative path，只在該 session lifecycle 內有效。Upload 可與 prepare 和 Agent execution 並行，但同一 session 一次只能有一個 upload。
...
`listAssets()` 會回傳 `assets/` 第一層的一般檔案，並依 filesystem modification time 由新到舊排序。此 method 不遞迴、不支援 pagination，Agent 執行期間仍可呼叫。Agent 寫入 nested file 時，請使用已知 path 搭配 `readAsset()`。
...
`"text"` 要求 valid UTF-8。`"json"` 會再將文字 parse 成 JSON；generic type 只提供 TypeScript annotation，使用不可信資料前仍應自行驗證。`"file"` 會將 binary bytes 保留在 browser `File`。Read 支援已知 nested `assets/...` path，單檔上限同樣為 100 MiB。
