...
App 與 Agent 共用 session 實體上的 `.pedelec-sandbox/assets/` 目錄。SDK 以 public `/...` path 提供這個 store，隱含根目錄不會出現在回傳 path。Session 結束前，App 可以 upload input file、遞迴列出所有已完成檔案，並讀取任一方產生的檔案。

### Upload 檔案

使用 `uploadAsset()` 傳送一個 browser `File`。單檔上限為 100 MiB；實體根目錄是 `.pedelec-sandbox/assets/`，公開 path 使用 `/...`，並以 `assets/` 為 SDK 隱含根目錄。Upload 可與 prepare 和 Agent execution 並行，但同一 session 一次只能有一個 upload。
