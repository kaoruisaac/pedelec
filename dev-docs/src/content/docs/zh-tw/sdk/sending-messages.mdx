...
Session 尚未結束時可使用 `uploadAsset()` 傳送一個 browser `File`。單檔上限為 100 MiB；回傳的 `assets/...` 是 sandbox-relative path。upload 可與 prepare、send、Agent 執行並行，但同一 session 一次只能有一個 upload。
...
`listAssets()` 不遞迴，並以 filesystem modification time 由新到舊排序，列出 App 與 Agent 共用的 `assets/`。`readAsset(path, "text" | "json" | "file")` 可讀取已知 nested path，binary bytes 走 loopback ticket data plane。
