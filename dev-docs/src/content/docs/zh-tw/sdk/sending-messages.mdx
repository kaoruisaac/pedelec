...
使用 `uploadAsset()` 傳送一個 browser `File`。單檔上限為 100 MiB；公開 path 使用 `/...`，並以 sandbox 的 `assets/` 為隱含根目錄。Upload 可與 prepare 和 Agent execution 並行，但同一 session 一次只能有一個 upload。
