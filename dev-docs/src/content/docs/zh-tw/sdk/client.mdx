...
不帶 path 的 `openWorkspace()` 是例外：原生 picker 開啟並等待使用者操作時，不套用 SDK bridge wall-clock timeout。帶有 explicit path 的 `openWorkspace(path)` 會套用設定的 bridge timeout；Extension/native disconnect 或明確的 bridge error 仍會立即 reject picker request。`workspace.run()` 也會把 timeout 交由 Core 決定，避免長時間 script 被一般 bridge timer 提前中止。
...
- `getSettings()` 與 `listProviders()` 需要 origin approval 且可能開啟 popup，且只公開 defaults 與 `name`/`code`/`available`/`isDefault`/`error`；為保持 forward compatibility，未知的 response 欄位會被忽略。
