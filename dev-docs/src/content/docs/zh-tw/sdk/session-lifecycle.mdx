...
Browser-side 的 `PedelecSession` handle 綁定於建立它的 SDK route。若該 runtime port disconnect，即使 parent `Pedelec` client 已 reconnect，這個 handle 仍會變成不可使用。請明確呼叫 `resumeSession(sessionId)`，讓新 handle subscribe 到仍存活的 Core session；SDK 不會靜默搬移舊 session operation。
...
## Resume 後的 provider/effort metadata
...
Reload 後 UI 需要 provider 或 effort level 時，請和 session ID 一起保存，或從 application-owned persistence 載入。不要假設 `resumeSession()` 會 rehydrate 所有 metadata。
...
`end()` 不會立即刪除 session sandbox；lifecycle 取決於建立 session 時的設定：

- 未指定 `sandbox.path`：使用 Desktop 管理的 temporary storage。Desktop App 正常退出時移除 managed sandbox，異常終止的 leftovers 會在下次啟動嘗試清除。關閉主視窗只會隱藏 App，不會執行這個 cleanup。
- 指定 absolute `sandbox.path`：使用 application 管理的 workspace。Pedelec 在 `end()`、App 正常退出或 stale-sandbox cleanup 時都不會刪除它，多個 active session 也可以共用。

即使 cleanup 尚未執行，ended handle 也不能透過 `uploadAsset()`、`listAssets()` 或 `readAsset()` 存取 asset。Shared workspace 的 filesystem conflict 不由 Pedelec 協調，explicit path 也不得與 managed sandbox root overlap。檔案被鎖定時，managed cleanup 可能要到後續啟動才能完成。
