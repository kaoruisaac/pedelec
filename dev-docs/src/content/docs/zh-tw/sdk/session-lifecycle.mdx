...
4. reconciliation 回傳的 authoritative lifecycle snapshot，包括 status、active/last operation identity，以及可能存在的 pending tool request；
...
Reload 後 UI 需要 provider 或 effort level 時，請和 session ID 一起保存，或從 application-owned persistence 載入。不要假設 `resumeSession()` 會 rehydrate 所有 metadata；但 `resumeSession()` resolve 後可依 `getStatus()` 取得 Core lifecycle state。
