...
Ending a session does not immediately delete its sandbox. Sandboxes are temporary Desktop-managed storage: they are removed on normal Desktop App exit, and leftover directories are cleaned up on the next app launch. Closing the main window only hides the app and does not trigger cleanup. Ended sessions cannot access retained sandbox assets through the asset APIs; forced termination or locked files can defer cleanup until a later launch.
