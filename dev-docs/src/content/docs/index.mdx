...
4. The target provider is available on the user's machine. CLI-backed providers use commands such as `codex`, `gemini`, `opencode`, `cursor`, `claude`, or official `grok`; the Ollama provider uses Pedelec's bundled `pedelec-agent`.
...
| Grok | `grok` | optional Grok CLI model ID |
...
type ProviderInfo = {
  name: string;
  code: "codex" | "gemini" | "opencode" | "cursor" | "claude" | "grok" | "ollama";
  path: string | null;
  available: boolean;
  error: string | null;
};
...
type PedelecSettings = {
  defaultProvider: "codex" | "gemini" | "opencode" | "cursor" | "claude" | "grok" | "ollama" | null;
  defaultModels: Partial<Record<"codex" | "gemini" | "opencode" | "cursor" | "claude" | "grok" | "ollama", string>>;
};
```
