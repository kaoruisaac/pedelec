...
### Specifying Provider, Model, and Effort
...
This is explicit model mode: the model is used directly and, when omitted, the provider chooses its default effort. Desktop profile mode instead uses `effortLevel` and the complete selected profile. Add provider-native `effort` only with `model`; do not combine explicit mode with `effortLevel`. Pedelec does not maintain a model catalog; model and effort validity remain provider-dependent.
...
Ollama sessions use the bundled `pedelec-agent` and the endpoint configured in Desktop Settings. Profile mode requires a model in the selected profile; an empty profile returns `MODEL_REQUIRED` and does not fall back to `default`. Explicit `{ model }` mode supplies the model directly, and Ollama does not currently accept explicit `effort`.
...
When only `provider` is passed, the SDK sends that provider with `effortLevel: "default"` and Core resolves the complete Desktop profile. Non-Ollama providers may have an empty profile; Ollama requires a model in its selected profile and returns `MODEL_REQUIRED` in profile mode. Use explicit `{ model, effort? }` mode for independent provider configuration; model-only requests use provider-default effort behavior.
...
`isDefault` identifies the provider selected by Desktop settings and is independent of `available`; an unavailable default provider still has `isDefault: true`. `available: false` usually means that the provider CLI is not installed or is not in `PATH`.
...
Ending a session does not immediately delete its workspace. Sessions created with `pedelec.createSession()` use temporary Desktop-managed storage: managed workspaces are removed on normal Desktop App exit, and leftover directories are cleaned up on the next app launch. An application-managed workspace is opened with `pedelec.openWorkspace(path)` and used through `workspace.createSession()`; it is never deleted by Pedelec on session end, app exit, or stale cleanup. Closing the main window only hides the app and does not trigger managed cleanup. Ended sessions cannot access retained workspace assets through the asset APIs, and explicit shared workspaces do not receive filesystem conflict coordination.
...
- Start with [Deno in Pedelec](./deno/overview/) to choose Agent scripts, Deno Modules, or `workspace.run()`.
