...
Error events include their responsibility domain at the event top level, outside the `error` object. `source` is either `"provider"` or `"core"`. A provider error always includes `provider`; valid provider codes are `"codex"`, `"antigravity"`, `"opencode"`, `"cursor"`, `"claude"`, and `"ollama"`. Core errors do not include `provider`.

```ts
type ProviderErrorEvent = {
  type: "error";
  source: "provider";
  provider: "codex" | "antigravity" | "opencode" | "cursor" | "claude" | "ollama";
