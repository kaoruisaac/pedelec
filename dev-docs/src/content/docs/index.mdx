...
`model` is optional. If omitted, Desktop's selected effort profile supplies the model and native effort arguments. If provided, Core replaces only that profile's model and keeps the selected native effort settings. Pedelec does not maintain a model catalog; model validity and account entitlement remain provider-dependent.
...
Ollama sessions use the bundled `pedelec-agent` and the endpoint configured in Desktop Settings. The selected profile must contain a model unless `createSession({ model })` supplies one; an empty selected profile otherwise returns `MODEL_REQUIRED` and does not fall back to `default`.
...
When only `provider` is passed, the SDK sends that provider with `effortLevel: "default"`. Non-Ollama providers may have an empty profile; Ollama requires a model in its selected profile and returns `MODEL_REQUIRED` otherwise, unless an explicit model override is supplied.
