...
Sessions created with `pedelec.createSession()` receive a temporary managed Workspace whose physical location and cleanup remain Desktop-owned; the SDK exposes its public `workspace.path` as `null`. An application-managed Workspace is selected with `pedelec.openWorkspace(path)` and used through `workspace.createSession()`. Pedelec initializes `.pedelec-runtime` for Workspace-owned runtime data: assets and logs are Workspace-level, generated Session skills and Deno Module state are isolated per Thread, and Workspace-run Deno Module packages are cached privately per Workspace and authorized SDK origin. Applications should not depend on the private `.pedelec-runtime` subdirectory layout. Pedelec preserves existing project content and private data and never deletes an application-managed Workspace. Multiple active Sessions may share it; filesystem write coordination remains the application's responsibility. Explicit paths cannot overlap the managed Workspace root. Workspace-run modules are mounted only for the explicit `workspace.run()` call and are not inherited by Sessions or Agent Host Context.
