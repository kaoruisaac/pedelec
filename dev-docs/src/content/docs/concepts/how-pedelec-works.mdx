...
- generates tool artifacts for the agent; and
...
Provider availability depends on the user's machine. CLI-backed providers usually require the relevant command to be installed, present in `PATH`, and authenticated. Ollama sessions use Pedelec's bundled agent executable but still require a reachable Ollama server and an installed model.
...
2. Core generated the agent-facing tool files in the session sandbox.
3. The agent decides that it needs the tool and issues a tool call.
