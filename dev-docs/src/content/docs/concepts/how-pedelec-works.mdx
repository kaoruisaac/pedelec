...
5. The desktop runtime resolves and validates the selected effort profile, snapshots its argv tokens, creates the session and sandbox, then returns a `sessionId`.
...
Without `sandbox.path`, the desktop runtime creates a temporary managed sandbox under `~/.pedelec/sandbox/<threadId>` and owns its cleanup. An application can instead pass an absolute `sandbox.path` to select an application-managed workspace. Pedelec creates `.pedelec-sandbox/{assets,logs,skills,tmp}/`, preserves existing project content and private data, and never deletes an explicit workspace. Multiple active sessions may share one explicit path; filesystem write coordination remains the application's responsibility. Explicit paths cannot overlap the managed sandbox root.
