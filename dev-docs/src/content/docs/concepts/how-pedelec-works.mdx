...
Pedelec has two Deno paths: the Agent runs JS/TS through `pedelec-deno`, while the Web application runs known Workspace code directly through `workspace.run()`. `skills.denoModules` supplies a Thread-scoped Agent module snapshot; Workspace-run modules belong only to runs that explicitly request them. Neither scope inherits the other. Desktop ships the restricted Deno runtime. See [Deno in Pedelec](../deno/overview/) for the choices and security boundary.
...
Sessions created with `pedelec.createSession()` receive a Desktop-managed Workspace whose public `workspace.path` is `null`. An application can open its own Workspace with `pedelec.openWorkspace(path)` and call `workspace.createSession()`. Multiple Sessions may share a Workspace; the application coordinates filesystem writes. Pedelec never deletes application-managed Workspaces. See [PedelecWorkspace](../reference/pedelec-workspace/) for public Workspace behavior.
