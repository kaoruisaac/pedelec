...
The application does not host instruction files. Core keeps the ToolRegistry as the runtime source of truth and injects `skills.guidance` plus tool names and descriptions into the first provider prompt. The initial prompt omits complete argument schemas; the agent uses `pedelec-cli tool-spec <tool-name>` when it needs one, then `pedelec-cli tool-call <tool-name> '<json_args>'` to execute. Core may still generate per-tool spec artifacts in the workspace, but the normal flow never requires reading `tools.md`.
