...
When `pedelec-cli` detects an IPC/transport failure for a `tool-call`, its structured error includes concise guidance to exact-retry the same listed call command with semantically identical arguments. Successful calls, normal structured App Tool errors, and formal `TOOL_TIMEOUT` responses do not receive this transport-retry guidance.
...
| Received formal `TOOL_TIMEOUT` | Do not treat it as an in-flight retry; reconcile possible side effects |
