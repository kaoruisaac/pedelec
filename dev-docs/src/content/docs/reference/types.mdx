...
`effortLevel` is provider-independent and may be used with or without `provider`. `model` is an optional provider-native identifier that overrides only the selected Desktop profile's model; it can be used with or without `provider` and `effortLevel`.
