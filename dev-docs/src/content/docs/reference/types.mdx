...
`entry` is a Vite build/dev authoring string. `name` becomes the Agent import specifier and `usage` is required guidance for the common import. Use `pedelecVitePlugin()` to prepare the runtime and declaration artifact.
