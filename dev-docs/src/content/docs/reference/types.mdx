...
This is the read-only snapshot returned by `sandboxFolderPicker()`. `hasSandboxConfig` only indicates that `.pedelec-sandbox.json` is a regular file; it does not validate the marker contents.
...
A sandbox-relative path in the session's shared `assets/` directory. Runtime validation also rejects backslashes, empty path segments, `.` and `..`.
