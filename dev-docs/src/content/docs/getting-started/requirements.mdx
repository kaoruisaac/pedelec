...
Ollama requires a model in the selected Desktop effort profile unless `createSession({ model })` supplies an explicit override. If both are absent, session creation fails with `MODEL_REQUIRED`; it does not fall back to another tier.
