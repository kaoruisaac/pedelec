...
The desktop application must be running when an operation requires Core Runtime access. It owns sessions, provider processes, settings, and the local runtime endpoint.

The extension may be reachable while the desktop application is not. In that case, approval status can still show the extension as installed, but provider or session operations fail when the native host cannot reach Core.
...
For Ollama, provider availability only confirms that Pedelec can find its bundled agent executable. It does not prove that the Ollama server is running or that the requested model exists.
...
5. The Desktop App remains running.
