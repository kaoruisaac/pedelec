...
## Provider and effort compatibility
...
Ollama profile mode requires a model in the selected Desktop effort profile. If it is absent, session creation fails with `MODEL_REQUIRED`; it does not fall back to another tier. Explicit `createSession({ model })` mode supplies the model directly, while Ollama does not currently accept explicit `effort`.
