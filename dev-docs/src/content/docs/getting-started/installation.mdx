...
- `CORE_RUNTIME_UNAVAILABLE` or `IPC_UNAVAILABLE`: the host cannot reach the running desktop Core.
...
- run an Ollama-compatible server;
...
## Local SDK development from this repository

When developing the SDK and application together before publishing a package version, install the repository's `sdk/` folder by path:
...
Then restart the application's development server so the bundler reloads the package output.

:::caution
A local path dependency only replaces the npm package. It does not automatically update or install the extension and desktop runtime.
:::

## Development origin

Use either `http://localhost:<port>` or `http://127.0.0.1:<port>`. The extension does not accept every plain-HTTP hostname.

Because approval is stored per origin, changing the port creates a different origin. Keep a stable development port when possible.
