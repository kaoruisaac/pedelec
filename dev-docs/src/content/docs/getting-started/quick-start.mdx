...
When only `provider` is supplied, the SDK uses Desktop profile mode with `effortLevel: "default"`; Core resolves that complete profile from Desktop Settings. You can pass `effortLevel: "low"` or `"high"` with or without an explicit provider. For independent provider configuration, pass `model` with an optional provider-native `effort`; this mode must not include `effortLevel`:
...
Model-only explicit sessions use the provider's default effort behavior. In Ollama profile mode the selected profile must contain a model; explicit `{ model }` mode supplies the model directly, and Ollama does not currently accept explicit `effort`.
