...
Pedelec does not allow any web page to create local agent sessions or open a native directory picker automatically. The Chrome Extension identifies the page origin and requires user approval before the first `createSession()`, `resumeSession()`, or `sandboxFolderPicker()` from that origin.
