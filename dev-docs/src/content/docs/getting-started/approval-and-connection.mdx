...
`getApprovalStatus()` only reads status and pings Core. It does not request approval. `getSettings()` and `listProviders()` are sensitive Desktop APIs: they require origin approval and can trigger the approval popup. They respectively expose only defaults, and only `name`/`code`/`available`/`error`; use `appConnected` rather than `listProviders()` for connectivity.
