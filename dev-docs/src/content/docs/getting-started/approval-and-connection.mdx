...
Pedelec does not allow any web page to create local agent sessions or open a native directory picker automatically. The Chrome Extension identifies the page origin and requires user approval before the first `createSession()`, `resumeSession()`, or `directoryPicker()` from that origin.
...
`getApprovalStatus()` only reads status and pings Core. It does not request approval. `getSettings()`, `listProviders()`, and `directoryPicker()` are sensitive Desktop APIs: they require origin approval and can trigger the approval popup. They expose only defaults, provider summaries, or the explicitly selected directory result; use `appConnected` rather than `listProviders()` for connectivity.
